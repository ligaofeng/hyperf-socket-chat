...
class WebSocketController extends BaseNamespace
{
    ...
    private function onSay(&$socket, &$data)
    {
        ...
        $this->service->push(['uid' => $this->getUid($socket), 'msg' => $data['data']['msg'], 'time' => date('Y-m-d H:i:s')]);
        Debug::create(['k' => 'endSync', 'v' => 1]);
        // 将当前用户加入房间
        $room = (string) $data['data']['room_id'];
        // 向房间内其他用户推送（不含当前用户）
        $user = User::findFromCache($this->getUid($socket));
        $emitData = json_encode(['type' => 1, 'data' => ['time' => date('Y-m-d H:i:s'), 'msg' => $user->name . '：' . $data['data']['msg']]]);
    ...
    private function onJoinRoom(&$socket, &$data)
    {
        ...
        $user = User::findFromCache($this->getUid($socket));
        // 向 本人 单点推送
        $emitData = json_encode(['type' => 1, 'data' => ['time' => date('Y-m-d H:i:s'), 'msg' => "{$user->name}，欢迎你进入了房间，当前房间共有" . $this->getRoomPersonNum() . '人']]);
        $socket->emit('event', $emitData);
        //给自己推送不要加->to，直接$socket->emit
        // 向房间内其他用户推送（不含当前用户）
        $emitData = json_encode(['type' => 1, 'data' => ['time' => date('Y-m-d H:i:s'), 'msg' => "{$user->name}进入了房间，当前房间共有" . $this->getRoomPersonNum() . '人']]);
        $socket->to($room)->emit('event', $emitData);
    }
    private static function jsonSuccess($data = [], $httpCode = 200)
    {
        return json_encode(['code' => (int) $httpCode, 'msg' => '', 'data' => $data]);
    }
    private static function jsonFail($msg, $httpCode, $code = 0)
    {
        return json_encode(['code' => $code == 0 ? $httpCode : $code, 'msg' => $msg, 'data' => []]);
    }
}